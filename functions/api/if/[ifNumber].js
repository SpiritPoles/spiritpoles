...
export async function onRequestGet({ params, env }) {
  ...
  try {
    ...
    // Build ship address
    let shipAddr = (ifRec.shipaddress || '').replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '').trim();
    if (!shipAddr) {
      const parts = [
        ifRec.shipaddressee,
        ifRec.shipaddr1,
        ifRec.shipaddr2,
        [ifRec.shipcity, ifRec.shipstate, ifRec.shipzip].filter(Boolean).join(', '),
        ifRec.shipcountry,
      ].filter(Boolean);
      shipAddr = parts.join('\n');
    }
    ...
    if (ifRec.createdfrom) {
      try {
        const soRes = await suiteQL(`
          SELECT tranid FROM transaction WHERE id = ${ifRec.createdfrom} LIMIT 1
        `, env);
